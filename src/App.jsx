    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 11, color: "#64748b", letterSpacing: 2, textTransform: "uppercase", marginBottom: 2 }}>Presupuesto</div>
            <div style={{ fontSize: 22, fontWeight: 700, color: "#f8fafc" }}>Familiar {año}</div>
          </div>
          ...
          <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 4 }}>
            {MESES.map((m, i) => (
              <button key={i} onClick={() => setMes(i)} style={{
                ...
                fontSize: 12, fontWeight: mes === i ? 700 : 400, transition: "all 0.2s",
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
          {[
            ...
            <div key={label} style={{
              ...
              <div style={{ fontSize: 15, fontWeight: 700, color }}>{val.toFixed(0)}€</div>
            </div>
          ))}
        </div>
      </div>
        ...
        <div style={{ padding: "16px 16px 0" }}>
          <div style={{ background: "#1e293b", borderRadius: 14, overflow: "hidden" }}>
            <div style={{ padding: "12px 16px", background: "#172554", borderBottom: "1px solid #1e3a8a" }}>
              <span style={{ fontWeight: 700, fontSize: 14 }}>Resumen Anual {año} — {PRESUPUESTOS[tab].label}</span>
            </div>
            {MESES.map((m, i) => {
              const t = calcTotales(data, tab, año, i);
              const hasDatos = t.ingresos > 0 || t.gastos > 0;
              return (
                <div key={i} onClick={() => { setMes(i); setVista("mes"); }} style={{
                  display: "flex", justifyContent: "space-between", alignItems: "center",
                  padding: "10px 16px", borderBottom: "1px solid #0f172a", cursor: "pointer",
            ...
            <div style={{ display: "flex", justifyContent: "space-between", padding: "12px 16px", background: "#0f172a" }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 700, color: disponibleColor(totalesAnuales.disponible) }}>{fmtDisplay(totalesAnuales.disponible)}</span>
            </div>
          </div>
        </div>
      )}
      ...
      {modalAbierto && editando && (
        <div style={{
          ...
          <div style={{
            background: "#1e293b", width: "100%", maxWidth: 480, borderRadius: "20px 20px 0 0",
            padding: "24px 20px 40px", border: "1px solid #334155"
            ...
            <div style={{ position: "relative", marginBottom: 20 }}>
              <input
                ...
                placeholder="0,00"
                style={{
                  width: "100%", padding: "16px 50px 16px 16px", fontSize: 20, fontWeight: 600,
                  background: "#0f172a", border: "2px solid #3b82f6", borderRadius: 12,
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <button onClick={() => setModalAbierto(false)} style={{
                padding: "14px", borderRadius: 12, border: "none", background: "#334155",
                color: "#94a3b8", fontSize: 15, fontWeight: 600, cursor: "pointer"
              }}>Cancelar</button>
              <button onClick={guardarEdicion} style={{
                padding: "14px", borderRadius: 12, border: "none", background: "#3b82f6",
    ...
    <div style={{ background: "#1e293b", borderRadius: 14, marginBottom: 12, overflow: "hidden" }}>
      ...
      {abierta && cats.map(cat => {
        ...
        return (
          <div
            key={cat}
            onClick={() => onTap(tipo, seccion, cat)}
            style={{
              display: "flex", justifyContent: "space-between", alignItems: "center",
              padding: "11px 16px", borderBottom: "1px solid #0f172a11", cursor: "pointer",
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{
                fontSize: 14, fontWeight: tieneValor ? 600 : 400,
